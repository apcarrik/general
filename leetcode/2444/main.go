...
//     // Find indicies for all instances of minK in nums
//     minKIndicies := []int{}
//     for i:=0; i<n; i++{
//         if nums[i] == minK {
//             minKIndicies = append(minKIndicies,i)
//         }
//     }
//     fmt.Println(minKIndicies)

//     // for each minK, search left then right for fixed-bound subarrays with minK & maxK as extreemes, until you hit
//     // another minK/maxK, keeping track of fixed-bound subarrays you encounter
//     fixedBoundCount := 0
//     for _,minKIdx := range minKIndicies {
//         max := 0
//         min := 1000001

//         leftLoop:
//         for i:=minKIdx-1; i>=0; i-- {
//             if nums[i] == minKIdx || min < minK || max < maxK {
//                 break leftLoop
//             }
//             if nums[i] > max {
//                 max = nums[i]
